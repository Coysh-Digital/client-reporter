                                ...
                                <li data-id="{{ $block->id }}" wire:key="block-{{ $block->id }}"
                                    ...
                                    <div x-show="open" x-cloak class="space-y-3 border-t border-line px-3 py-3">
                                        ...
                                        @if (! $type || $type->supportsCommentary())
                                            @php
                                                // Built as plain strings so the tag braces render literally as
                                                // text, rather than Blade compiling them when the value is echoed.
                                                $mergeTagList = collect(\App\Support\MergeTags::available())
                                                    ->keys()
                                                    ->map(fn (string $tag): string => '{{ '.$tag.' }}')
                                                    ->implode(', ');
                                                $mergeHelp = 'Personalise this note with merge tags — '.$mergeTagList.' — filled in automatically when the report generates.';
                                                $mergePlaceholder = 'A note shown under this section — e.g. Hi {{ client }},';
                                            @endphp
                                            <x-field label="Commentary" :for="'block-'.$block->id.'-commentary'" :name="'edits.'.$block->id.'.commentary'" optional :help="$mergeHelp">
                                                <textarea wire:model="edits.{{ $block->id }}.commentary" id="block-{{ $block->id }}-commentary" wire:blur="persistBlock({{ $block->id }})"
                                                          rows="2" placeholder="{{ $mergePlaceholder }}" class="cr-input text-sm"></textarea>
                                            </x-field>
                                        @endif
