@include('reports.blocks.partials.heading', ['text' => $heading ?: \App\Support\ReportLang::get('analytics.heading'), 'icon' => $icon ?? 'chart', 'suffix' => $data['provider'] ?? null])

@if (! ($data['has_data'] ?? false) || empty($data['metrics']))
    <p class="muted">{{ \App\Support\ReportLang::get('common.empty.analytics') }}</p>
@else
